...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 21 22:34:20 2021
    ...
    for j in range(N):
        if i != j:
            ...
            if r_number > 0.6:
                ...
                empty_dict[i,j] = 0   
inflow = empty_dict.copy()   
             
#Sorting the Demand Matrix
sorted_values = sorted(empty_dict.values(), reverse=True) # Sort the values
sorted_dict = {}

for i in sorted_values:
    for k in empty_dict.keys():
        if empty_dict[k] == i:
            sorted_dict[k] = empty_dict[k]
            break

demand_matrix = sorted_dict.copy()       
...
optic_arcs = general_optic.copy()

#print("\nGeneral list is the following:",optic_arcs,"\n")
...
for d in demand_matrix.keys():
    ...
    if demand_matrix[d] > 0:
        ...
        total_cost = total_cost + demand_matrix[d] * shortest[0]
